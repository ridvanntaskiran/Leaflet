...
function createSidebarElements(layer) {
    const el = `<div class="sidebar-el" id="${layer._leaflet_id}" data-marker="${layer._leaflet_id}">${layer
        .getLatLng()
        .toString()}<button class="btn-danger col-12" onclick="DeleteLocation(${layer._leaflet_id})" style="float:right;min-width:40px;margin-right:5px"><i class="fas fa-trash-alt" style="margin-right:5px;"></i>Sil</button></div>`;
...
var _cookie = getCookie('Locations');
...
var dlAnchorElem = document.getElementById('downloadLocations');
function LocationSave() {
    ...
    id += 1;
    const marker = L.marker([lat.toFixed(5), lng.toFixed(5)]).addTo(fg);
    createSidebarElements(marker);
    map.fitBounds(fg.getBounds());
    setCookie('Locations', JSON.stringify(points), 30);
...
function DeleteLocation(markerId) {
    var myobj = document.getElementById(markerId);
    myobj.remove();
    const marker = fg.getLayer(markerId);
    map.removeLayer(marker);
}

if ((!_cookie || _cookie == undefined || _cookie == "" || _cookie.length == 0)) {
    dlAnchorElem.style.display = 'none';
}
else {
    dlAnchorElem.style.display = 'inline';
    let cookieJson = JSON.parse(_cookie);
    for (var i = 0; i < cookieJson.length; i++) {
        var latitude = cookieJson[i].lat;
        var longitude = cookieJson[i].lng;
        var id = cookieJson[i].id;
        var datetime = cookieJson[i].date;
        const marker = L.marker([latitude, longitude]).addTo(fg);
        createSidebarElements(marker);
        map.fitBounds(fg.getBounds());
        points.push({
            "id": id,
...
function DownloadLocations() {
    var dataStr = "data:text/json;charset=utf-8," + encodeURIComponent(JSON.stringify(_cookie));
    
    dlAnchorElem.setAttribute("href", dataStr);
    dlAnchorElem.setAttribute("download", "locations.json");
}
